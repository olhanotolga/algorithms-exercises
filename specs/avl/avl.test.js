...
class Node {
  ...
  add(value) {
    // decide to go left or right (call .add() on the left or right child)
    if (value < this.value) {
      if (this.left) {
        this.left.add(value);
      } else {
  ...
  balance() {
    // check if this node is out of balance (by the difference between left and right heights)
    const rightHeight = this.right ? this.right.height : 0;
    const leftHeight = this.left ? this.left.height : 0;

    // if it's out of balance, decide of signle or double rotation
    // if single - call rotate on self,
    // if double - call rotate on child, and then on self
    if (leftHeight > rightHeight + 1) {
      const leftRightHeight = this.left?.right ? this.left.right.height : 0;
      const leftLeftHeight = this.left?.left ? this.left.left.height : 0;

      // the subtree is "bent" => double rotation:
  ...
  // called if the right child is heavy
  rotateRR() {
  ...
  // called if the left child is heavy
  rotateLL() {
    const valueBefore = this.value;
